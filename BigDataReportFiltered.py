...
def preprocess(rawDataRDD, stopwords, topSubjectEbookIDs):
    # Use regex to capture header & footer & bookID
    # Subset only matched ebooks & flatmap
    matchedBooksRDD = (rawDataRDD
                       .map(main_text)
                       .filter(lambda x: x[0] is not None)
                       .filter(lambda x: x[0] in topSubjectEbookIDs))
    ...
    # Reduce by book to get total wordcounts & save to disk
...
if __name__ == "__main__":
    ...
    for chunk in grouper(dirs, batchSize):
        ...
        bookWordCountRDD, chunk_count = preprocess(rawDataRDD, stopwords, topSubjectEbookIDs)
        doc_count += chunk_count
        bookWordCount.extend(bookWordCountRDD.collect())
    ...
    # print("TfIdfRDD: ", TfIdfRDD.first(), "\n")
    print("TfIdfRDD count: ", TfIdfRDD.count())
    # Build vocabulary list
    # vocab = wordBookCountRDD.keys().distinct().collect()
    # Use hashing trick
    hashSize = 10000
    hashVecRDD = TfIdfRDD.map(lambda x: b_hashVector(x, hashSize))
    # print("hashVecRDD: ", hashVecRDD.first(), "\n")
    ...
    # print("subjectIdRDD: ", subjectIdRDD.collect(), "\n")
    # Keep only top 20 subjects' Vectors
    # Replace subject string by subject id
    topSubjectVecRDD = (subjectVecRDD
                        .join(subjectIdRDD)
                        .map(lambda x: (x[1][1], x[1][0])))
    print("topSubjectVecRDD size: ", topSubjectVecRDD.count(), "\n")
    # Create labeled points
    lblPnt = topSubjectVecRDD.map(lambda x_l: LabeledPoint(x_l[0], x_l[1])).collect()

    # Save as pickle file
    with open("lblPnt.p", "wb") as f:
        pickle.dump(lblPnt.p, f)
    # Load pickle file
    with open("lblPnt.p", "rb") as f:
        lblPnt = pickle.load(f)
    # Bring to spark context
    lblPntRDD = sc.parallelize(lblPnt)
